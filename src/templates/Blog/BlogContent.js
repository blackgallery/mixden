          ...
          BlogData.reverse().slice(0, 3).map((post, i) => (
            <div className="blog-warp-1 blog_warp_lay_1" key={i}>
              ...
              <div className="blog_content_warp">
                <Link to="#/" className="blog_datee"><i
                  className="fa fa-calendar"></i> {post.publishDate} </Link>
                <Link to="#/" className="subtext"><i className="fa fa-tag"></i> {post.tag} </Link>
                <Link to={`${`/blog/${post.title.split(" ").join("-").toLowerCase()}?id=${post.id}`}`}
                      className="subtext"><i className="fa fa-comment-o"> </i> {post.commentCount}
                </Link>
                <h5><Link
                  to={`${`/blog/${post.title.split(" ").join("-").toLowerCase()}?id=${post.id}`}`}> {post.title}</Link>
                </h5>
                <p> {post.content}  </p>
                <Link to={`${`/blog/${post.title.split(" ").join("-").toLowerCase()}?id=${post.id}`}`}
                      className="more-link"> Read More </Link>
              </div>
