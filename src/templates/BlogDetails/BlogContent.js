      ...
      <div className="col-md-8 col-sm-12 blog-area">
        <div className="blog-warp-1 blog_warp_lay_1">
          ...
          <div className="blog_content_warp">
            ...
            <a href={`/blog/${post.title.split(" ").join("-").toLowerCase()}?id=${post.id}`}
               className="subtext"><i className="fa fa-comment-o"> </i> {post.commentCount} </a>
            <h5><a
              href={`/blog/${post.title.split(" ").join("-").toLowerCase()}?id=${post.id}`}>{post.title}</a>
            </h5>
            <p> {post.content} </p>
          ...
          <ul className="comments">
            <li>
              <div className="comment">
                <img src={require("../../assets/images/" + post.comments.proPic)} alt="blog"
                     className="comment-avatar"/>
                <div className="blog_com_dt">
                  <strong className="commenter-title"><a href="#/">{post.comments.name}</a><span
                    className="comment-date">{post.comments.date}</span></strong>
              ...
              <ul>
                <li>
                  <div className="comment">
                    <img src={require("../../assets/images/" + post.comments.proPic)} alt="service"
                         className="comment-avatar"/>
                    <div className="blog_com_dt">
                      <strong className="commenter-title"><a href="#/">{post.comments.name}</a><span
                        className="comment-date">{post.comments.date}</span></strong>
