              ...
              <div className="col-md-4 col-sm-12" key={id}>
                <div className="service_list">
                  ...
                  <div className="service_para">
                    <i className="icon_easel"/> <h5> {service.title} </h5>
                    <p>{service.shortDesc}</p>

                    <Link
                      to={`${`/service/${service.title.split(" ").join("-").toLowerCase()}?id=${service.id} `}`}
                      className="srvic_read">Read More <span className="fa fa-angle-double-right"/></Link>
                  </div>
                </div>
              </div>
            ))
